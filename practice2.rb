...
def isogram_matcher(isogram1, isogram2)
  ...
  isogram1.chars.each_index do |idx|
    if isogram1[idx] == isogram2[idx]
      exact_matches += 1
    elsif isogram2.include?(isogram1[idx])
      displaced_matches += 1
    end
  end

  return [exact_matches, displaced_matches]
end
...
def panoramic_pairs(landmarks)
  ...
  landmarks.each_index do |idx|
    if idx == landmarks.length - 1
      panoramics << [landmarks[idx], landmarks[0]]
    else
      panoramics << [landmarks[idx], landmarks[idx+1]]
    end
  end
...
def xbonacci(starting_sequence, number_of_xbonacci_numbers_to_return)
  nums_to_add = starting_sequence.length
  while starting_sequence.length < number_of_xbonacci_numbers_to_return
    starting_sequence << starting_sequence.last(nums_to_add).reduce(:+)
  end
