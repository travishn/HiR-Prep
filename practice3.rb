...
def in_all_strings?(long_strings, short_string)
  long_strings.all? { |target_string| target_string.include?(short_string) }
end
...
def biodiversity_index(specimens)
  ...
  smallest = specimens.min_by { |species| specimens.count(species) }
  largest = specimens.max_by { |species| specimens.count(species) }

  # realized I can simply make a hash
  return num_species**2 * (specimens.count(smallest).to_f / specimens.count(largest))
end
...
def first_last_indices(str)
  ...
  str.chars.each_with_index do |letter, idx|
    if letter_hash[letter].length < 2
      letter_hash[letter] << idx
    elsif letter_hash[letter].length == 2 && letter_hash[letter][1] < idx
      letter_hash[letter][1] = idx
    end
  end
